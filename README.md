...
| QA Agent | Godot 4.7, headless + optional NVIDIA video QA | Imports assets and runs each level (which also catches compile errors), then **plays it**: Autoplay holds each arrow key and compares motion against an idle baseline. For `collect`, `dot_maze`, and `maze_chase`, a second harness-owned probe preflights pickup placement, moves the real player to every spawned pickup (flood-filling live wall geometry for mazes), follows an unlocked exit when present, and requires the generated script to enter its actual `won` state. Its structured result includes completion time, progress events, longest stall, stuck state, deaths, restart applicability, and a completion score. Static balance and screenshot review follow. With `SAGA_VIDEO_QA=1`, Godot also records the complete eight-second autoplay sequence, FFmpeg creates a compact MP4, and NVIDIA Nemotron reviews temporal evidence including motion, facing direction, animation, HUD readability, jitter and disappearing objects. Every attempt and artifact is retained in `run.json`; unresolved gating defects cannot become clean passes |
...
Each maps to the structurally nearest of six worked few-shot examples in `coder.py`, since showing a local model a complete example of the structure it's asked to produce remains its biggest reliability lever. `dot_maze`'s few-shot is the largest (244 lines) and routes to a bigger model via `TEMPLATE_MODEL_OVERRIDES` - the 14B reliably dropped variable declarations at that length.
...
- Generic autoplay still finds the floor, not the ceiling: it proves a player responds to input but does not reach an objective. `collect`, `dot_maze`, and `maze_chase` now have deterministic completion probes that verify pickup interaction, reachability, optional exit traversal, the real win state, completion time, progress stalls, and stuck runs. The remaining templates still need equivalent mechanic-specific solvers; an `ordered_switches` puzzle is next, while a herding game whose creatures could never actually be pushed anywhere remains the motivating failure case.
