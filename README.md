...
| QA Agent | Godot 4.7, headless + optional NVIDIA video QA | Imports assets and runs each level (which also catches compile errors), then **plays it**: Autoplay holds each arrow key and compares motion against an idle baseline. For `collect`, `dot_maze`, and `maze_chase`, a harness-owned probe moves the real player to every pickup (flood-filling live wall geometry for mazes) and requires the actual `won` state. For `ordered_switches`, it activates one correct switch followed by an intentional wrong one, requires progress to reset, reloads the scene cleanly, then completes the authored sequence and requires the real win state. Structured results include completion time, progress events, longest stall, stuck state, deaths, restart behavior, sequence metrics, and a completion score. Static balance and screenshot review follow. With `SAGA_VIDEO_QA=1`, Godot also records the complete eight-second autoplay sequence, FFmpeg creates a compact MP4, and NVIDIA Nemotron reviews temporal evidence including motion, facing direction, animation, HUD readability, jitter and disappearing objects. Every attempt and artifact is retained in `run.json`; unresolved gating defects cannot become clean passes |
...
- Generic autoplay still finds the floor, not the ceiling: it proves a player responds to input but does not reach an objective. `collect`, `ordered_switches`, `dot_maze`, and `maze_chase` now have deterministic completion probes. The switch probe verifies both rejection/reset of a wrong sequence and successful completion from a clean reload. The remaining templates still need equivalent mechanic-specific solvers; `survive_hazards` is next, while a herding game whose creatures could never actually be pushed anywhere remains the motivating failure case.
