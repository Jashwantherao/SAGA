...
| QA Agent | Godot 4.7, headless + optional NVIDIA video QA | Imports assets and runs each level (which also catches compile errors), then **plays it**: Autoplay holds each arrow key and compares motion against an idle baseline. For `collect`, `dot_maze`, and `maze_chase`, a harness-owned probe moves the real player to every pickup (flood-filling live wall geometry for mazes) and requires the actual `won` state. For `ordered_switches`, it verifies a wrong-order reset, reloads cleanly, then completes the authored sequence. For `survive_hazards`, it requires one collision to remove exactly one life, repeats valid hits until the real lose state, presses the actual restart input and validates fresh state, then shortens only the public timer and requires the real win transition. Structured results include completion time, progress events, longest stall, stuck state, damage, terminal losses, restart behavior, mechanic metrics, and a completion score. Static balance and screenshot review follow. With `SAGA_VIDEO_QA=1`, Godot also records the complete eight-second autoplay sequence, FFmpeg creates a compact MP4, and NVIDIA Nemotron reviews temporal evidence including motion, facing direction, animation, HUD readability, jitter and disappearing objects. Every attempt and artifact is retained in `run.json`; unresolved gating defects cannot become clean passes |
...
- Generic autoplay still finds the floor, not the ceiling: it proves a player responds to input but does not reach an objective. `collect`, `ordered_switches`, `survive_hazards`, `dot_maze`, and `maze_chase` now have deterministic completion probes. Survival QA exercises both lose/restart and timer-win paths rather than merely waiting for a label. The remaining templates still need equivalent mechanic-specific solvers; `depletion` is next, while a herding game whose creatures could never actually be pushed anywhere remains the motivating failure case.
