...
| QA Agent | Godot 4.7, headless + optional NVIDIA video QA | Imports assets and runs each level (which also catches compile errors), then **plays it**: Autoplay holds each arrow key and compares motion against an idle baseline. Mechanic probes now complete pickups and mazes, reject and solve ordered switches, exercise survival damage/lose/restart/win paths, verify depletion and hybrid resource loops, and prove capture-zone progress, contest decay, ownership reset, and all-owned victory. They move the real player through real Area2D interactions and require actual state transitions, not label changes. Structured results include completion time, progress events, longest stall, stuck state, damage, resource/fuel/territory deltas, terminal losses, restart behavior, mechanic metrics, and a completion score. Static balance and screenshot review follow. With `SAGA_VIDEO_QA=1`, Godot also records the complete eight-second autoplay sequence, FFmpeg creates a compact MP4, and NVIDIA Nemotron reviews temporal evidence including motion, facing direction, animation, HUD readability, jitter and disappearing objects. Every attempt and artifact is retained in `run.json`; unresolved gating defects cannot become clean passes |
...
Each maps to the structurally nearest of eight worked few-shot examples in `coder.py`, since showing a local model a complete example of the structure it's asked to produce remains its biggest reliability lever. `ordered_switches` and `capture_zones` have dedicated examples because autonomous QA requires stable sequence, progress, decay, and ownership interfaces. `dot_maze`'s few-shot is the largest (244 lines) and routes to a bigger model via `TEMPLATE_MODEL_OVERRIDES` - the 14B reliably dropped variable declarations at that length.
...
- Generic autoplay still finds the floor, not the ceiling: it proves a player responds to input but does not reach an objective. `collect`, `ordered_switches`, `survive_hazards`, `depletion`, `survive_and_deplete`, `capture_zones`, `dot_maze`, and `maze_chase` now have deterministic completion probes—8 of 9 mechanic templates. Resource and territory QA measure live behavior rather than trusting advertised rates or labels. Only `herd_to_goal` still needs an equivalent mechanic-specific solver; a herding game whose creatures could never actually be pushed anywhere remains the motivating failure case.
