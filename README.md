...
- Ollama's runner (`llama-server`) intermittently crashes on model load with a Windows CUDA-init failure (`0xc0000409`), particularly noticeable when the Coder's model loads right after a large ComfyUI image batch. The crash rate appears to *increase* across repeated model loads within one Ollama session rather than being a one-time VRAM-handover race, which ruled out the obvious fixes (a settle delay after freeing ComfyUI's VRAM, escalating retry backoff up to 60s) as a complete solution - they measurably recover some runs but not all. Not yet resolved; `OLLAMA_KEEP_ALIVE=-1` (keep the model resident instead of letting it unload/reload between calls) is the next thing to try.
