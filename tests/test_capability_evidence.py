...
def test_real_action_rpg_manifest_probes_are_all_backed_by_parser_evidence(monkeypatch):
    output = "\n".join([
        ...
        "[ACTION_RPG_PLAYTHROUGH] status=passed movement=true melee=true pickup=true inventory=true dialogue=true quest=true rooms=true checkpoint=true dash=true boss_phase=true win=true frames=3000 attacks=28 interactions=3 deaths=0 reason=none",
    ])
    monkeypatch.setattr(
        qa_agent,
        "_run",
